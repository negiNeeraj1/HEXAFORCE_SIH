...
exports.getUserNotifications = async (req, res) => {
  try {
    ...
    let userNotifications = notifications.filter((notification) => {
      ...
      if (notification.recipients === "students" && req.user.role === "user")
        return true;
      if (notification.recipients === "admins" && req.user.role === "admin")
        return true;
      if (
        notification.recipients === "specific" &&
        notification.specificUsers &&
        notification.specificUsers.includes(userId)
      )
        return true;
    ...
    const notificationsWithReadStatus = userNotifications.map(
      (notification) => ({
        ...notification,
    ...
    notificationsWithReadStatus.sort(
...
exports.getUnreadCount = async (req, res) => {
  try {
    ...
    const unreadCount = notifications.filter((notification) => {
      ...
      if (notification.recipients === "users") targetsUser = true;
      if (notification.recipients === "admins" && req.user.role === "admin")
        targetsUser = true;
      if (
        notification.recipients === "specific" &&
...
exports.getAnalytics = async (req, res) => {
  try {
    ...
    const totalRead = notifications.reduce(
      (sum, n) => sum + (n.deliveryStats?.read || 0),
      0
    );
