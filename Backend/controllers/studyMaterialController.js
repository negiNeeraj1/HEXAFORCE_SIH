...
exports.getMaterialById = async (req, res, next) => {
  try {
    ...
    if (!material) {
      return next(
        new ErrorResponse(`Study material not found with id of ${id}`, 404)
      );
    }
...
exports.uploadMaterial = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin") {
      return next(
        new ErrorResponse("Not authorized to upload study materials", 403)
      );
    }
    ...
    const newMaterial = {
      ...
      description,
      category,
      fileUrl: `/uploads/study-materials/${title
        .toLowerCase()
        .replace(/\s+/g, "-")}.pdf`,
...
exports.deleteMaterial = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin") {
      return next(
        new ErrorResponse("Not authorized to delete study materials", 403)
      );
    }
...
exports.updateMaterial = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin") {
      return next(
        new ErrorResponse("Not authorized to update study materials", 403)
      );
    }
