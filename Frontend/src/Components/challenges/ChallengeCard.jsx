...
const ChallengeCard = ({ challenge, onPress, onVerifyChallenge }) => {
  const handleVerifyClick = (e) => {
    e.stopPropagation();
    onVerifyChallenge(challenge);
  };
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className="h-30 relative overflow-hidden">
          {challenge.isSystemImage ? (
            <div className="w-full h-full bg-gradient-to-br from-green-100 to-blue-100 flex items-center justify-center">
              <i
                className={`${challenge.imageName} text-6xl text-green-600`}
              ></i>
            </div>
          ) : (
            <ChallengeImage
        ...
        {/* Challenge Status */}
        {challenge.isStarted ? (
          <div className="bg-green-100 text-black text-sm text-center py-1.5 px-3 flex items-center justify-center gap-2">
            <CheckCircle size={16} />
            Challenge Completed
          </div>
        ) : (
          <div className="bg-blue-100 text-blue-800 text-sm text-center py-1.5 px-3 flex items-center justify-center gap-2">
            <Camera size={16} />
            Click to Verify
          </div>
        )}
        ...
        {!challenge.isStarted && (
          <button
            ...
            title="Verify Challenge Completion"
          >
            <Camera size={16} />
          </button>
        )}
